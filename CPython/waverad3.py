...
parser.add_argument("-d", "--dark-theme", action="store_true",
                    help="Dark background for plots")
parser.add_argument("-a", "--all-returns", action="store_true",
                    help="Print all radar returns, not just first one")
...
# The last seen hour, used to detect midnight and update what day it is
last_hour = None

# Flag to indicate we've just detected midnight and not had a GPS fix today
midnight_no_fix_yet = False
...
for line in lines:
    ...
    # be fixed, applying the discovered date to previous lines.
        ...
        try:
            # Get time of day as a time.struct_time object
            when_struct = time.strptime(parts[0], "%Y-%m-%dT%H:%M:%S")
...
print(f"Data: {len(heights_meters)} points")

# If asked to write just times and tide heights into a simple CSV, do so now.
# This CSV is designed to be read by an FFT program such as wavefft1.py.
if args.write_csv:

    csv_file_name = path.splitext(args.input_file)[0] + "_tide.csv"
    print(f"Creating CSV file '{csv_file_name}'")

    # Convert date/time values to seconds after the beginning
    begin_time = datetimes[0][0].timestamp()
    dt_epoch = [t.timestamp() - begin_time for t in datetimes[0]]

    with open(csv_file_name, 'w') as csv_file:
        csv_file.write("Time\tTide Height\r\n")
        for time, tide in zip(dt_epoch, heights_meters):
            csv_file.write(f"{time},{tide:.3f}\r\n")

# If plots haven't been turned off, make and display the plot
if not args.no_plots:

    # Enable dark background if the correct parameter has been set
    if args.dark_theme:
    ...
    figs, axes = pyplot.subplots(nrows=2, ncols=1, 
                                ...
                                sharex=True)
    if args.all_returns:
        for time_series, dist_series, strn_series in zip(datetimes, distances, strengths):
            print(f"Plot: {len(time_series)} vs. {len(dist_series)} points")
            axes[0].plot(time_series, dist_series, '.', markersize=2)
            axes[1].plot(time_series, strn_series, '.', markersize=2)
    else:
        axes[0].plot(datetimes[0], heights_feet, '.', markersize=2)
        axes[1].plot(datetimes[0], strengths[0], '.')

    axes_m = axes[0].twinx()      # Axis on the right for height in meters

    for ax in axes: ax.grid(True, linestyle='--')
    axes[0].set_title("Radar Distance Measurements")
    axes[0].xaxis.set_major_formatter(myFmt)
    if args.all_returns:
        axes[0].set_ylabel("Distance from Sensor (ft)")
        axes[0].set_ylim(0, PLOT_Y_MAX)
        axes_m.set_ylabel("Distance from Sensor (m)")
        axes_m.set_ylim(0, PLOT_Y_MAX / FEET_PER_METER)
    else:
